...
interface PedidoCardProps {
  pedido: {
    _id: string;
    tipo: 'local' | 'recoger' | 'domicilio'; // ✅ AÑADIDO
    mesa?: { // ✅ OPCIONAL
      _id: string;
      numero: number;
    };
    ...
    impuestos: number;
    descuento: number;
    gastoEnvio?: number; // ✅ AÑADIDO
    total: number;
    estado: 'pendiente' | 'preparando' | 'listo' | 'en_camino' | 'servido' | 'entregado' | 'pagado' | 'cancelado';
    camarero?: {
      nombre?: string;
      email: string;
    };
...
export default function PedidoCard({
  ...
  const getSiguienteEstado = () => {
    ...
    if (pedido.tipo === 'local') {
      switch (pedido.estado) {
        ...
        case 'listo': return 'servido';
        case 'servido': return 'pagado';
        default: return null;
      }
    }
  ...
  const getSiguienteEstadoTexto = () => {
    if (pedido.tipo === 'local') {
      switch (pedido.estado) {
        ...
        case 'preparando': return '→ Listo';
        case 'listo': return '→ Servir';
        case 'servido': return '→ Cobrar';
        default: return null;
      }
    }
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 border-2 border-gray-700 hover:border-amber-500 transition">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          {/* ✅ Mostrar tipo e info según corresponda */}
          ...
          {pedido.cliente && (
            <p className="text-sm text-gray-300 font-semibold">{pedido.cliente}</p>
          )}
          
          {pedido.telefono && (
            <p className="text-xs text-gray-400">📞 {pedido.telefono}</p>
          )}

          {pedido.direccionEntrega && (
            <p className="text-xs text-gray-400 mt-1">
              📍 {pedido.direccionEntrega.calle} {pedido.direccionEntrega.numero}, {pedido.direccionEntrega.ciudad}
            </p>
          )}
        </div>
        <span
          className={`${getEstadoColor()} text-white px-4 py-2 rounded-full text-sm font-semibold`}
        >
          {getEstadoTexto()}
        </span>
      </div>
      ...
      <div className="mb-4 space-y-2">
        ...
        {pedido.productos.map((item, idx) => (
          <div
            key={idx}
            className="flex justify-between text-sm bg-gray-700 px-3 py-2 rounded"
          >
      ...
      <div className="mb-4 text-xs text-gray-400 space-y-1">
        {pedido.camarero && (
          <p>👤 Camarero: {pedido.camarero.nombre || pedido.camarero.email}</p>
        )}
        {pedido.metodoPago && (
          <p>💳 Método pago: {pedido.metodoPago}</p>
        )}
      ...
      <div className="flex gap-2">
        {siguienteEstado && (
          <button
            onClick={() => onCambiarEstado(pedido._id, siguienteEstado)}
