...
export default function PedidoCard({
  ...
  return (
    ...
    >
      {/* ── Header: estado + id + tipo + tiempo ── */}
      <div className="flex items-center justify-between px-3 pt-3 pb-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className={`w-2 h-2 rounded-full ${estadoCfg.dot}`} />
          <span className="text-xs font-semibold text-gray-200">{estadoCfg.label}</span>
          <span className="text-[10px] text-gray-500 font-mono">#{pedido._id.slice(-4).toUpperCase()}</span>
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${TIPO_BADGE[pedido.tipo]}`}>
            {TIPO_ICON[pedido.tipo]} {TIPO_LABEL[pedido.tipo]}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-gray-400">{new Date(pedido.createdAt).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</span>
      ...
      <div className="px-3 pb-2">
        ...
        {pedido.tipo === 'recoger' && (
          <div>
            <p className="text-sm font-bold text-white truncate">{pedido.cliente || 'Cliente'}</p>
            {pedido.telefono && <p className="text-[11px] text-gray-400 mt-0.5">{pedido.telefono}</p>}
          </div>
        )}
        {pedido.tipo === 'domicilio' && (
          <div>
            ...
            {pedido.direccionEntrega && (
              <p className="text-[11px] text-gray-400 mt-0.5 truncate">{pedido.direccionEntrega.calle} {pedido.direccionEntrega.numero}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex gap-1.5 px-3 pb-3" onClick={(e) => e.stopPropagation()}>
        {siguiente && (
          <button
            ...
            className="flex-1 py-2 bg-green-600 hover:bg-green-700 text-white rounded font-semibold transition text-xs active:scale-95"
        ...
        {whatsappUrl && (
          <a
            ...
            className="px-2.5 py-2 bg-[#25D366] hover:bg-[#1da851] text-white rounded transition text-xs flex items-center justify-center active:scale-95"
        ...
        {puedeEditar && onEditar && (
          <button
            ...
            className="px-2.5 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded transition text-xs"
            title="Editar"
          >
            ✏️
          </button>
        ...
        {onVerDetalle && (
          <button
            ...
            className="px-2.5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition text-xs"
            title="Ver detalle"
          >
            👁
          </button>
        ...
        {puedeCancelar && (
          <button
            ...
            className="px-2.5 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition text-xs"
            title="Cancelar"
          >
            ✕
          </button>
